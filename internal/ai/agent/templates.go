...
func BuildPlanChangesPrompt(ticketKey, ticketSummary, ticketDescription, repoContext string, opts PlanPromptOptions) string {
	rules := []string{
		...
		`Only use operation=edit on a file that is shown ABOVE in this exact prompt with its full content. Never guess, reconstruct, or assume the content of a file - including a file you know exists from context like a README, or one shown signatures-only. If you need to edit any file whose full content is not shown above, respond with ONLY {"need_files":["relative/path.ext", ...]} (no array, no other keys) and nothing else - you will be given the full content and asked again. This applies even if the file isn't mentioned anywhere in the provided context yet - if the ticket or repo conventions imply it exists (e.g. a variables file that must declare a new toggle), ask for it by its likely path rather than fabricating an edit against content you haven't seen.`,
